...
{ iquest "%s/%s" "SELECT COLL_NAME, DATA_NAME where COLL_NAME like '%$LIMSID%' and DATA_NAME like '%GRCh38.small_variants.phased.vcf.gz%'" | grep 'fam\|cohort' > $OUT; } || { iquest "%s/%s" "SELECT COLL_NAME, DATA_NAME where COLL_NAME like '%$LIMSID%' and DATA_NAME like '%GRCh38.deepvariant.phased.vcf.gz%'" | grep 'fam\|cohort' > $OUT; } || { iquest "%s/%s" "SELECT COLL_NAME, DATA_NAME where COLL_NAME like '%$LIMSID%' and DATA_NAME like '%GRCh38.deepvariant.glnexus.phased.vcf.gz%'" | grep 'fam\|cohort' > $OUT; } || { iquest "%s/%s" "SELECT COLL_NAME, DATA_NAME where COLL_NAME like '%$LIMSID%' and DATA_NAME like '%GRCh38.small_variants.phased.vcf.gz%'" | grep -v 'fam\|cohort' | grep -E '\.vcf\.gz(\.tbi)?$' > $OUT; }

# retrieve joint-genotyped SV VCF
{ iquest "%s/%s" "SELECT COLL_NAME, DATA_NAME where COLL_NAME like '%$LIMSID%' and DATA_NAME like '%GRCh38.structural_variants.phased.vcf.gz%'" | grep 'fam\|cohort' >> $OUT; } || { iquest "%s/%s" "SELECT COLL_NAME, DATA_NAME where COLL_NAME like '%$LIMSID%' and DATA_NAME like '%GRCh38.pbsv.phased.vcf.gz%'" | grep 'fam\|cohort' >> $OUT; } || { iquest "%s/%s" "SELECT COLL_NAME, DATA_NAME where COLL_NAME like '%$LIMSID%' and DATA_NAME like '%GRCh38.structural_variants.phased.vcf.gz%'" | grep -v 'fam\|cohort' | grep -E '\.vcf\.gz(\.tbi)?$' >> $OUT; }
